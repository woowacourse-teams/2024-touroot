...
class TraveloguePhotoTest {
    ...
    void setUp() {
        ...
        TravelogueDay firstDay = TravelogueDayFixture.FIRST_DAY.getTravelogueDayIncludedIn(travelogue);
        traveloguePlace = TraveloguePlaceFixture.MANJANG_CAVE.getTraveloguePlaceIncludedIn(firstDay);
    }

    @DisplayName("올바른 여행 장소 사진을 생성 시에는 예외가 발생하지 않는다")
    @Test
    void createTraveloguePhotoWithValidData() {
        assertThatCode(() -> new TraveloguePhoto(VALID_ORDER, VALID_KEY, traveloguePlace))
    ...
    void createTraveloguePhotoWithNullOrder() {
        assertThatThrownBy(() -> new TraveloguePhoto(null, VALID_KEY, traveloguePlace))
    ...
    void createTraveloguePhotoWithNullKey() {
        assertThatThrownBy(() -> new TraveloguePhoto(VALID_ORDER, null, traveloguePlace))
                .isInstanceOf(BadRequestException.class)
                .hasMessage("여행 장소의 사진에서 순서와 키, 그리고 사진이 속한 여행 장소는 비어 있을 수 없습니다");
    }
    ...
    void createTraveloguePhotoWithNegativeOrder(int negative) {
        assertThatThrownBy(() -> new TraveloguePhoto(negative, VALID_KEY, traveloguePlace))
                .isInstanceOf(BadRequestException.class)
